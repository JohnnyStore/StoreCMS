var app = new Vue({
  ...
  methods:{
    ...
    onChange: function (rowIndex) {
      ...
      app.$data.itemHotID = $(row).find('td').eq(0).text();
      ...
      app.$data.startDate = $.trim($(row).find('td').eq(4).text()).substring(0, 10);
      app.$data.endDate = $.trim($(row).find('td').eq(5).text()).substring(0, 10);
    ...
    onDelete: function (itemHotID, itemName) {
      var confirmMsg = '您确定要删除热销商品：' + itemName + '吗？';
      bootbox.confirm(confirmMsg, function(result) {
        if(result) {
          $.ajax({
            url: '/itemHot?itemHotID=' + itemHotID,
            type: 'delete',
            dataType: 'json',
    ...
    onSearch: function () {
      location.href = '/itemHot?itemID=' + this.search_itemID
