$(document).ready(function () {
  var _newsTextEditType = 'add';
  var _updPartObj = null;
  var _partID = 0;
  var _saveType = 'add';
  var _updNewsID = 0;
  function initPage() {
  ...
  function showData(){
    ...
    var saveType = $('#hidden-saveType').val();
    _saveType = saveType;
    _updNewsID = newsID;
    if(saveType === 'upd'){
      $.ajax({
  ...
  function addNewsPart(newsContent, newsType){
    ...
    $('.detail .part[id=part' + _partID + ']').on("click","a.part-text", function() {
      ...
      var newsText = $(this).parent().parent().find('p').text();
    ...
    $('.detail .part[id=part' + _partID + ']').on("click","a.part-remove", function() {
      var currentPart = $(this).parent().parent();
      var currentPartType = $(this).parent().parent().attr('data-type');
      if(currentPartType === 'I'){
        var imageUrl = $(currentPart).find('img').attr('src');
    ...
    $('.detail .part[id=part' + _partID + ']').on("click","a.part-down", function() {
      var currentPartObj = $(this).parent().parent();
  ...
  function moveUp(obj){
    //1 取得前一个part元素
    var preObj = $(obj).prev('div.part');
    if(preObj.length === 0){
      alertMessage('这部分已经在最前面啦。');
      return false;
    }
    //2 将当前元素插入到上一个元素之前
    $(preObj).before(obj);
  ...
  function moveDown(obj){
    //1 拿到下一个part元素
    var nextObj = $(obj).next('div.part');
    if(nextObj.length === 0){
      alertMessage('这部分已经在最后面啦。');
      return false;
    }
    //2 将当前元素插入到下一个元素之后
    $(nextObj).after(obj);
  ...
  $('#btn-add-text').click(function () {
    var newsText = $.trim($('#from-field-news-text').val());
    if(newsText.length === 0){
      alertMessage('请输入新闻的文字内容');
      return false;
    }
  ...
  $('#btn-add-image').click(function () {
    ...
    var fileList = uploadTools.uploadedList;
  ...
  function checkData(){
    var newsTitle = $.trim($('#form-field-newsTitle').val());
    var newsDate = $.trim($('#id-date-picker-1').val());
    var newsThumbnailUrl = $('div.news-title-thumbnail img').attr('src');
    var newsContentCount = $('div.detail').children().length;

    if(newsTitle.length === 0){
      alertMessage('请填写新闻标题。');
      return false;
    }
    if(newsDate.length === 0){
      alertMessage('请填写新闻日期。');
      return false;
    }

    if(newsThumbnailUrl === '/images/upload/demo.jpg'){
      alertMessage('请添加新闻标题图片。');
      return false;
    }

    if(newsContentCount === 0){
      alertMessage('请添加新闻内容（文字或者图片）。');
      return false;
    }
    return true;
  }
  ...
  function saveNews(){
    ...
    var newsDate = $.trim($('#id-date-picker-1').val());
    var newsThumbnailUrl = $('div.news-title-thumbnail img').attr('src');
    var newsContentArray = $('div.detail').children('div.part');
    var newsContentObj = [];
      ...
      $.ajax({
        ...
        success: function(res){
          if(res.err){
            ...
            alertMessage('保存成功, 3秒后返回新闻列表页面', {icon: 6});
